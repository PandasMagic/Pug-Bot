...
class DeployCommand implements Command {
  ...
  async execute(client: Client, state: BotState,serverState: ServerState, message: Message, args: string[]): Promise<void> {
    ...
    let clandata = await state.clanClient.getClanData(serverState.clanid);
    serverState.clanstate.update(clandata);

    let targetMember = serverState.clanstate.clanData.members.find((elem) => { return elem.name.toLocaleLowerCase() == targetUser.toLocaleLowerCase() });
    if (!targetMember) {
    ...
    let toUndeploy = this.getPlayerToUndeploy(serverState.clanstate.clanData.members)
    targetMember.deployed = true;
    targetMember.lastDeployed = Date.now()
    if (toUndeploy) {
      toUndeploy.deployed = false;
    }
    state.clanClient.updateClanStatus(serverState.clanstate.generateClanStatus());
